...
## NAFLDiet_signChangedAssociation.R
library("psych")
library(ppcor)
library(gplots)
...
rbind(M0.Kruskal.changedList, M12.Kruskal.changedList) %>% distinct()
...
load(file = "sp_Kruskal_re_ab_deltaM120.rda")
load(file = "sp_Kruskal_re_ab_changedBothM0M12_deltaM120.rda")
...
###### Part 1 #####################
## use Kruskal testing  for these changed at M12 but not M0 ## 

sp_Kruskal_re_ab_deltaM120 %>% t() %>% as.data.frame() %>% rownames_to_column() %>%
...
### 26 deltafat, without deltaFat 
for (i in 1:18){
  for (j in c(20:25)){
    var1 <- colnames(df1)[i]
    var2 <- colnames(df1)[j]
    value <-  pcor.test(df1[,i], df1[,j],  df1[,c(19)], method="spearman")
...
for (i in 1:18){
  for (j in c(20:26)){
    var1 <- colnames(df2)[i]
    var2 <- colnames(df2)[j]
    value <-  pcor.test(df2[,i], df2[,j],  df2[,c(19)], method="spearman")
    rbind(data.frame(var1=var1, var2 = var2, value=value), pcor.test.result) -> pcor.test.result
  }
}
...
tree <- as.treedata(taxa.18, include.rownames=TRUE)
...
pcor.test.result.all %>% filter(! is.na(value.estimate)) %>%
  arrange(desc(abs(value.estimate))) -> df3

df3 %>% dplyr::filter(grepl("Fimenecus",species))

df3$var2 <-gsub("3","",as.character(df3$var2))
df3$var2 <-gsub(".cholesterol","",as.character(df3$var2))
df3$var2 <-gsub("delta","Fat",as.character(df3$var2))
...
partial.r.result %>% colnames()
as.data.frame.matrix(partial.r.result)[1:18,19:25] %>% rownames_to_column(var = "sp")-> tmp.df
tmp.df
### tree circle ### 
...
circos.initialize("a", xlim = c(0, 25)) # only one sector
...
#########################################################################################################
########## Part II
######### changed at both M0 and M12
########
################################################################################################################

sp_Kruskal_re_ab_changedBothM0M12_deltaM120 %>% t() %>% as.data.frame() %>% rownames_to_column() %>%
  separate_wider_delim(cols = rowname, delim = "-",names = c("cID", "Month")) %>%
  separate_wider_delim(cols = cID, delim = "_",names = c("Prefix", "ID")) %>%
  left_join(sample.127.388.liver.delta.3) %>%
  mutate(ID=as.character(ID)) %>%
...
for (i in 1:57){
  for (j in c(59:65)){
    var1 <- colnames(df2)[i]
    var2 <- colnames(df2)[j]
    value <-  pcor.test(df2[,i], df2[,j],  df2[,c(58)], method="spearman")
    rbind(data.frame(var1=var1, var2 = var2, value=value), pcor.test.result) -> pcor.test.result
  }
}
...
pcor.test.result.all %>% colnames()

tax.table.dict[rownames(tax.table.dict) %in% pcor.test.result.all$var1,] %>% dplyr::select(-nTax) %>%
  arrange(phylum) -> taxa.58


as.data.frame.matrix(partial.r.result) %>% rownames()
## tree circle ### 
as.data.frame.matrix(partial.r.result)[1:57,58:64] %>% rownames_to_column(var = "sp")-> tmp.df
tmp.df
tax.table.dict[rownames(tax.table.dict) %in% tmp.df$sp,] %>% rownames_to_column(var = "sp1") %>%
  left_join(tmp.df, by=c("sp1" = "sp"), keep = T) -> tmp0.df
tmp0.df
...
tmp0.df %>% select(1:11)  %>% as.matrix() -> tmp0.1.df
taxa58 <- phyloseq::tax_table(tmp0.1.df)


library(phytools)

tree58 = rtree(ntaxa(taxa58), rooted=TRUE, tip.label=taxa_names(taxa58))
tree58 = rtree(ntaxa(taxa58), rooted=TRUE, tip.label=tmp0.df$nTax)

tree58 <- force.ultrametric(tree58, method = "extend")

is.ultrametric(tree58)
hc <- as.hclust.phylo(tree58)
labels
labels = hc$labels  # 
ct = cutree(hc, 5)  # cut tree into 5 pieces
n = length(labels)  # 
...
tmp0.df %>% select(1:11)  %>% dplyr::select(phylum) %>% distinct()
...
pdf("try.pdf")
circos.par(cell.padding = c(0, 0, 0, 10))
#circos.par(gap.degree=c(0,0,0,0,10))
...
circos.track(ylim = c(0, 1), bg.border = NA, track.height = 0.3, 
            ...
            # draw.sector(360,10,
            #             rou1 = get.cell.meta.data("cell.top.radius", 
...
suppressPackageStartupMessages(library(dendextend))
dend = color_branches(dend, k = 5, col = 1:5)
dend_height = attr(dend, "height")
circos.track(ylim = c(0, dend_height), bg.border = NA, 
             track.height = 0.4, panel.fun = function(x, y) {
               circos.dendrogram(dend)
...
as.data.frame.matrix(partial.r.result)[1:57,58:64]

as.data.frame.matrix(partial.r.result)[1:57,58:64] %>% rownames_to_column(var = "sp")-> tmp.df

M0.Kruskal.changedList
...
mat %>% arrange(phylum) -> mat
split = factor(mat$phylum, levels = mat$phylum)

dev.off()
mat[,2:8] -> mat1
dend.1 = as.dendrogram(hc)
function(dend, m, si) dend <- dend.1
...
circos.heatmap.initialize(mat, split = split)
col_fun1 = colorRamp2(c(-0.3, 0, 0.3), c("red", "white", "blue"))
circos.heatmap(mat1[,4:7],  col = col_fun1)#,rownames.side = "inside")
circos.track(track.index = get.current.track.index(), panel.fun = function(x, y) {
  get.cell.meta.data("ycenter")
  #circos.text(CELL_META$xcenter, CELL_META$ycenter, CELL_META$sector.index)
  if(CELL_META$sector.numeric.index == 1) { # the last sector
    cn = colnames(mat1[,4:7])
    n = length(cn)
    circos.text(rep(CELL_META$cell.xlim[1]-1.2, n), 
                1:n, cn, 
                cex = 0.5, adj = c(0, 1.2), facing = "bending.inside")
  }
}, bg.border = NA)

circos.heatmap(mat1[,1:3],  col = col_fun1)#,rownames.side = "inside")
circos.track(track.index = get.current.track.index(), panel.fun = function(x, y) {
  get.cell.meta.data("ycenter")
  #circos.text(CELL_META$xcenter, CELL_META$ycenter, CELL_META$sector.index)
  if(CELL_META$sector.numeric.index == 1) { # the last sector
    cn = colnames(mat1[,1:3])
    n = length(cn)
    circos.text(rep(CELL_META$cell.xlim[1]-1.2, n), 
                1:n, cn, 
                cex = 0.5, adj = c(0, 1.2), facing = "bending.inside")
...
circos.heatmap.initialize(mat, split = split)
col_fun1 = colorRamp2(c(-0.3, 0, 0.3), c("red", "white", "blue"))
circos.heatmap(mat1,  col = col_fun1)#,rownames.side = "inside")
circos.track(track.index = get.current.track.index(), panel.fun = function(x, y) {
  get.cell.meta.data("ycenter")
  #circos.text(CELL_META$xcenter, CELL_META$ycenter, CELL_META$sector.index)
  if(CELL_META$sector.numeric.index == 1) { # the last sector
